...
app.get('/health/smtp', async (req, res) => {
  ...
  const to = (req.query.to as string) ?? user
  ...
  try {
    ...
    if (req.query.send === '1' && to) {
      await transporter.sendMail({
        from, to,
        subject: 'Fantasy12 — teste SMTP produção',
        text: 'Se você recebeu este email, o SMTP está funcionando em produção.',
      })
      return res.json({ ok: true, host, port, user, sent: true, to })
    }

    return res.json({ ok: true, host, port, user, sent: false })
  } catch (err: unknown) {
