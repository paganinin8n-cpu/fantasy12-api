...
-- Database-only invariant: at most one OPEN round.
CREATE UNIQUE INDEX IF NOT EXISTS "rounds_single_open_idx"
ON "rounds" ((1))
WHERE "status" = 'OPEN';

-- Database-only financial invariants.
ALTER TABLE "wallets"
...
-- Database-only bolao invite invariants.
